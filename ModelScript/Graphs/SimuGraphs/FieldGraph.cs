...
namespace ModelScript.Graphs.SimuGraphs
{
    public class FieldGraph : SimulationGraphBase
    {
        ...
        public override void render(int width, int height, ref SKCanvas canvas)
        {
            ...
            if (plane[0] == 'X')
            {
                matrixwidth = space.x / matrixWidth;
            }

            if (plane[0] == 'Y')
            {
                matrixwidth = space.y / matrixWidth;
            }

            if (plane[0] == 'Z')
            {
                matrixwidth = space.z / matrixWidth;
            }

            if (plane[1] == 'X')
            {
                matrixheight = space.x / matrixHeight;
            }

            if (plane[1] == 'Y')
            {
                matrixheight = space.y / matrixHeight;
            }

            if (plane[1] == 'Z')
            {
                matrixheight = space.z / matrixHeight;
            }
            ...
            for (int x = 0; x < matrixWidth; x++)
            {
                for (int y = 0; y < matrixHeight; y++)
                {
                    ...
                    foreach (var p in particleList)
                    {
                        ...
                        if (plane[0] == 'X')
                        {
                            val1 = (int)((p.position.x - minPoint.x) / matrixwidth);
                        }

                        if (plane[0] == 'Y')
                        {
                            val1 = (int)((p.position.y - minPoint.y) / matrixwidth);
                        }

                        if (plane[0] == 'Z')
                        {
                            val1 = (int)((p.position.z - minPoint.z) / matrixwidth);
                        }

                        if (plane[1] == 'X')
                        {
                            val2 = matrixHeight - (int)((p.position.x - minPoint.x) / matrixheight);
                        }

                        if (plane[1] == 'Y')
                        {
                            val2 = matrixHeight - (int)((p.position.y - minPoint.y) / matrixheight);
                        }

                        if (plane[1] == 'Z')
                        {
                            val2 = matrixHeight - (int)((p.position.z - minPoint.z) / matrixheight);
                        }
